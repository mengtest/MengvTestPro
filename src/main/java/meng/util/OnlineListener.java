	...
	public void attributeAdded(HttpSessionBindingEvent evt) {
		...
		if (ConfigUtil.getSessionInfoName().equals(name)) {// 如果存入的属性是sessionInfo的话
			...
			if (sessionInfo != null) {
				...
				TonlineServiceI tonlineService = (TonlineServiceI) ctx.getBean("tonlineServiceImpl");
				Tonline online = new Tonline();
				online.setType("1");// 登录
				online.setLoginname(sessionInfo.getTuser().getLoginname());
				online.setIp(sessionInfo.getTuser().getIp());
				tonlineService.save(online);
			}
		}
	}
