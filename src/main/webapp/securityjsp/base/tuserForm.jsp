...
<script type="text/javascript">
	...
	var submitNow = function($dialog, $grid, $pjq) {
		...
		if ($(':input[name="data.id"]').val().length > 0) {
			url = mydef.path + '/base/tuser!update.sy';
		} else {
			url = mydef.path + '/base/tuser!save.sy';
		}
		$.post(url, mydef.serializeObject($('form')), function(result) {
			parent.mydef.progressBar('close');//关闭上传进度条
	...
	$(function() {
		...
		if ($(':input[name="data.id"]').val().length > 0) {
			...
			$.post(mydef.path + '/base/tuser!getById.sy', {
				...
				if (result.id != undefined) {
					...
					if (result.photo) {
						$('#photo').attr('src', mydef.path + result.photo);
					}
				}
		...
		uploader = new plupload.Uploader({//上传插件定义
			...
			flash_swf_url : mydef.path + '/jslib/plupload-2.1.1/plupload/js/plupload.flash.swf',// Flash环境路径设置
			url : mydef.path + '/plupload?fileFolder=/userPhoto',//上传文件路径
		...
		uploader.bind('UploadProgress', function(up, file) {//上传进度改变
			...
			parent.mydef.progressBar({//显示文件上传滚动条
				title : '文件上传中...',
				value : msg
			});
		});
