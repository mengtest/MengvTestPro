...
<script type="text/javascript">
	var submitForm = function($dialog, $grid, $pjq, $mainMenu) {
		if ($('form').form('validate')) {
			...
			if ($(':input[name="data.id"]').val().length > 0) {
				url = mydef.path + '/base/tresource!update.td';
			} else {
				url = mydef.path + '/base/tresource!save.td';
			}
	...
	var showIcons = function() {
		var dialog = parent.mydef.modalDialog({
			title : '浏览小图标',
			url : mydef.path + '/style/icons.jsp',
			buttons : [ {
				text : '确定',
				handler : function() {
	...
	$(function() {
		if ($(':input[name="data.id"]').val().length > 0) {
			...
			$.post(mydef.path + '/base/tresource!getById.td', {
		...
		<fieldset>
			...
			<table class="table" style="width: 100%;">
				...
				<tr>
					...
					<th>资源类型</th>
					<td><select name="data.tresourcetype.id" class="easyui-combobox" data-options="required:true,editable:false,valueField:'id',textField:'name',url:'<%=path%>/base/tresourcetype!doNotNeedSecurity_combobox.td',panelHeight:'auto'" style="width: 155px;"></select></td>
				</tr>
				<tr>
					<th>上级资源</th>
					<td><select id="tresource_id" name="data.tresource.id" class="easyui-combotree" data-options="editable:false,idField:'id',textField:'text',parentField:'pid',url:'<%=path%>/base/tresource!doNotNeedSecurity_getMainMenu.td'" style="width: 155px;"></select><img class="iconImg ext-icon-cross" onclick="$('#tresource_id').combotree('clear');" title="清空" /></td>
					<th>资源图标</th>
