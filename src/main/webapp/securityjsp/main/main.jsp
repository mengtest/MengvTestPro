	...
	$(function() {
		...
		var loginFun = function() {
			if ($('#loginDialog form').form('validate')) {
				...
				$.post(mydef.path + '/base/tuser!doNotNeedSessionAndSecurity_login.td', $('#loginDialog form').serialize(), function(result) {
					if (result.success) {
			...
			buttons : [ {
				...
				handler : function() {
					if ($('#passwordDialog form').form('validate')) {
						$.post(mydef.path + '/base/tuser!doNotNeedSecurity_updateCurrentPwd.td', {
							'data.pwd' : $('#pwd').val()
						}, function(result) {
							if (result.success) {
		...
		mainMenu = $('#mainMenu').tree({
			url : mydef.path + '/base/tresource!doNotNeedSecurity_getMainMenu.td',
			parentField : 'pid',
			onClick : function(node) {
				if (node.attributes.url) {
					var src = mydef.path + node.attributes.url;
					if (!mydef.startWith(node.attributes.url, '/')) {
					...
					if (node.attributes.target && node.attributes.target.length > 0) {
						...
						var opts = {
							...
							content : mydef.formatString('<iframe src="{0}" allowTransparency="true" style="border:0;width:100%;height:99%;" frameBorder="0"></iframe>', src),
							border : false,
							fit : true
						};
		...
		$('#mainLayout').layout('panel', 'center').panel({
			onResize : function(width, height) {
				mydef.setIframeHeight('centerIframe', $('#mainLayout').layout('panel', 'center').panel('options').height - 5);
			}
		});
...
<body id="mainLayout" class="easyui-layout">
	<div data-options="region:'north',href:'<%=contextPath%>/securityjsp/main/north.jsp'" style="height: 70px; overflow: hidden;" class="logo"></div>
	...
	<div data-options="region:'south',href:'<%=contextPath%>/securityjsp/main/south.jsp',border:false" style="height: 30px; overflow: hidden;"></div>
	...
	<div id="loginDialog" title="解锁登录" style="display: none;">
		<form method="post" class="form" onsubmit="return false;">
			<table class="table">
				<tr>
					<th width="50">登录名</th>
					<td><%=sessionInfo.getTuser().getLoginname()%><input name="data.loginname" readonly="readonly" type="hidden" value="<%=sessionInfo.getTuser().getLoginname()%>" /></td>
				</tr>
				<tr>
					<th>密码</th>
	...
	<div id="passwordDialog" title="修改密码" style="display: none;">
		<form method="post" class="form" onsubmit="return false;">
			<table class="table">
				<tr>
					<th>旧密码</th>
					<td><input id="pwd" name="data.oldpwd" type="password" class="easyui-validatebox" data-options="required:true" /></td>
				</tr>
				<tr>
					<th>新密码</th>
