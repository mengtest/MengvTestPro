var mydef = mydef || {};
mydef.data = mydef.data || {};// 用于存放临时的数据或者对象
 ...
 * 增加命名空间功能
 * 
 * 使用方法：mydef.ns('jQuery.bbb.ccc','jQuery.eee.fff');
 */
mydef.ns = function() {
	var o = {}, d;
 ...
 * 将form表单元素的值序列化成对象
 * 
 * @example mydef.serializeObject($('#formId'))
 * 
 * @requires jQuery
 * 
 * @returns object
 */
mydef.serializeObject = function(form) {
	var o = {};
 ...
 * 增加formatString功能
 * 
 * @example mydef.formatString('字符串{0}字符串{1}字符串','第一个变量','第二个变量');
 * 
 * @returns 格式化后的字符串
 */
mydef.formatString = function(str) {
 ...
 * @returns list
 */
mydef.stringToList = function(value) {
...
mydef.jsonToString = function(o) {
	...
	if (typeof o == "object") {
		if (!o.sort) {
			for ( var i in o)
				r.push(i + ":" + mydef.jsonToString(o[i]));
 ...
 * 
 * @example mydef.cookie('the_cookie', 'the_value');
 * @desc Set the value of a cookie.
 * @example mydef.cookie('the_cookie', 'the_value', { expires: 7, path: '/', domain: 'jquery.com', secure: true });
 * @desc Create a cookie with all available options.
 * @example mydef.cookie('the_cookie', 'the_value');
 * @desc Create a session cookie.
 * @example mydef.cookie('the_cookie', null);
 ...
 * 
 * @example mydef.cookie('the_cookie');
 ...
 * @author Klaus Hartl/klaus.hartl@stilbuero.de
 */
mydef.cookie = function(key, value, options) {
