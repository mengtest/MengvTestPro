...
/**
 * Panel关闭时候回收内存，主要用于layout使用iframe嵌入网页时的内存泄露问题
...
$.extend($.fn.dialog.defaults, mydef.onMove);
$.extend($.fn.window.defaults, mydef.onMove);
$.extend($.fn.panel.defaults, mydef.onMove);
...
$.extend($.fn.datagrid.defaults, mydef.onLoadError);
$.extend($.fn.treegrid.defaults, mydef.onLoadError);
$.extend($.fn.tree.defaults, mydef.onLoadError);
$.extend($.fn.combogrid.defaults, mydef.onLoadError);
$.extend($.fn.combobox.defaults, mydef.onLoadError);
$.extend($.fn.form.defaults, mydef.onLoadError);
...
$.extend($.fn.combotree.defaults, mydef.loadFilter);
$.extend($.fn.tree.defaults, mydef.loadFilter);
 ...
 * 
 */
mydef.modalDialog = function(options) {
	var opts = $.extend({
		title : '&nbsp;',
		width : 640,
		height : 480,
 ...
 * @param themeName
 */
mydef.changeTheme = function(themeName) {
	...
	mydef.cookie('easyuiTheme', themeName, {
		expires : 7
	});
};
 ...
 * @requires jQuery,EasyUI
 */
mydef.progressBar = function(options) {
